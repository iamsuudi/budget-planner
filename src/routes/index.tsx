      ...
      <main className="pt-24 pb-32 px-6 max-w-5xl mx-auto space-y-10">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              ...
              className="w-10 h-10 flex items-center justify-center rounded-xl bg-surface-container border border-white/5 text-secondary hover:text-secondary-fixed transition-all"
            >
            ...
            <button
              ...
              disabled={!canGoNext}
              className="w-10 h-10 flex items-center justify-center rounded-xl bg-surface-container border border-white/5 text-secondary hover:text-secondary-fixed transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          {monthBudget && monthBudget.totalBudget > 0 && (
            <div className="hidden md:flex gap-2">
              <span
                className={`px-4 py-1 rounded-full text-sm font-semibold ${budgetPercentage >= 100 ? 'bg-error/10 text-error' : 'bg-tertiary/10 text-tertiary'}`}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="grid grid-cols-2 gap-6 border-t border-white/10 pt-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <p
                      className={`text-2xl font-bold ${(monthBudget?.totalBudget || 0) - totalExpenses >= 0 ? 'text-tertiary' : 'text-error'}`}
                    >
                      {formatAmount(
                        Math.max(
          ...
          <div className="flex flex-col gap-6">
            <Link
              to="/expense/add"
              className="flex-1 bg-primary-container text-on-primary-fixed-variant rounded-[1.5rem] p-6 flex flex-col items-start justify-between shadow-lg glow-violet hover:brightness-110 active:scale-95 transition-all"
            >
            ...
            <Link
              to="/budget"
              className="flex-1 border border-secondary text-secondary rounded-[1.5rem] p-6 flex flex-col items-start justify-between hover:bg-secondary/10 active:scale-95 transition-all"
            >
              <span className="material-symbols-outlined text-4xl">
                payments
              </span>
        ...
        {monthBudget && monthBudget.totalBudget > 0 && (
          <GlassCard>
            <div className="flex justify-between items-center mb-2">
              ...
              <p
                className={`text-sm font-semibold ${budgetPercentage >= 100 ? 'text-error' : 'text-tertiary'}`}
              >
                {budgetPercentage.toFixed(0)}% of $
                {monthBudget.totalBudget.toLocaleString()} used
              </p>
            </div>
            <ProgressBar
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {invoices.slice(0, 10).map((inv) => (
                <div
                  key={inv.id}
                  className="glass-card flex items-center justify-between p-4 rounded-xl hover:bg-surface-container-high transition-colors"
                >
                  <div className="flex items-center gap-4">
